...
# col: 파이 그림의 색깔 지정
# cex: 범주 글자 크기 지정
freq_table <- table(death) ; freq_table
pie(freq_table, label=cause, main='사망원인에 대한 원형 그래프',
    col=c('red','yellow'),    
    cex=0.7)

pie(freq_table, label=cause, main='사망원인에 대한 원형 그래프', 
    col=rainbow(7), cex=1.2)
...
barplot(height=freq_table, col=2, 
...
legend(x='right', legend=cause,
       lty=c(-3), pch=c(-2), fill=rainbow(10),
...
pareto.result <- pareto.chart(data=freq_table, main='사망원인', 
...
age
age1 <- cut(age, breaks=c(9,19,29,39)) #이산형의 범주화
age1
levels(age1)
...
# 빈도수 기준(4개의 코드 모두 같은 결과)
hist.result <- hist(drink)
hist.result <- hist(drink, freq=NULL)
hist.result <- hist(drink, freq=T)
hist.result <- hist(drink, prob=F)  # prob의 default= !freq

# density 기준(2개의 코드 모두 같은 결과)
...
hist.result
summary(hist.result)
lines(x=hist.result$mids, y=hist.result$counts, type='b', pch=20, lty = 2)


# type b: line and point type c: without point
# density 기준
hist.result <- hist(drink, prob=T)
lines(x=hist.result$mids, y=hist.result$density, type='b', pch=5)
